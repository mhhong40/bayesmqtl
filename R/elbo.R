...
elbo_z_rho_ <- function(X, z_vb, log_Phi_xi_vb, log_1_Phi_xi_vb, sig2_gam_0_vb, sig2_gam_1_vb) {
  ...
  eps <- .Machine$double.eps # Control log argument when z_vb or 1 - z_vb is very small

  elbo_no_entropy_z <- sweep(z_vb * log_Phi_xi_vb + (1 - z_vb) * log_1_Phi_xi_vb - 1/2 * (X^2 %*% sig2_gam_1_vb), 2, 1/2 * sig2_gam_0_vb, "-")

  entropy_z <- - (z_vb * log(z_vb + eps) + (1 - z_vb) * log(1 - z_vb + eps))
...
elbo_lambda_ <- function(a_inv_vb, log_a_inv_vb, nu_lambda, eta_lambda, log_lambda_inv2_vb, lambda_inv2_vb) {

  elbo_lambda <- 1/2 * log_a_inv_vb + (1/2 - nu_lambda)*log_lambda_inv2_vb - (a_inv_vb - eta_lambda)*lambda_inv2_vb - nu_lambda*log(eta_lambda)
  elbo_lambda <- sum(elbo_lambda)

  return(elbo_lambda)
}

elbo_tau_ <- function(b_inv_vb, log_b_inv_vb, nu_tau, eta_tau, log_tau_inv2_vb, tau_inv2_vb) {

  elbo_tau <- 1/2 * log_b_inv_vb + (1/2 - nu_tau)*log_tau_inv2_vb - (b_inv_vb - eta_tau)*tau_inv2_vb - nu_tau*log(eta_tau)
  elbo_tau <- sum(elbo_tau)

  return(elbo_tau)
}

elbo_a_ <- function(c_a, d_a, a_inv_vb, log_a_inv_vb) {

  elbo_a <- (1/2 - c_a)*log_a_inv_vb - (1 - d_a)*a_inv_vb - c_a*log(d_a)
  elbo_a <- sum(elbo_a)

  return(elbo_a)
}

elbo_b_ <- function(c_b, d_b, b_inv_vb, log_b_inv_vb) {

  elbo_b <- (1/2 - c_b)*log_b_inv_vb - (1 - d_b)*b_inv_vb - c_b*log(d_b)
  elbo_b <- sum(elbo_b)
