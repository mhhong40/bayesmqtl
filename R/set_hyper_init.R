...
auto_set_init_ <- function(Y, p = 1, user_seed) {
  ...
  mu_gam_1_vb <- rnorm(d, mean = 0, sd = 1)

  tau_inv2_vb <- rgamma(1, shape = max(p, d), rate = 1)
  lambda_inv2_vb <- rgamma(d, shape = max(p, d), rate = 1) # Might have to revise, but keeping it the same as tau_inv2_vb for now since both have the same inv. Gamma parametrization
